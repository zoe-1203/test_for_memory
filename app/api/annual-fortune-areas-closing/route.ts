...
export async function POST(req: Request) {
  ...
  try {
    ...
    setImmediate(() => {
      try {
        const promptFileName = `areas_closing_${timestamp}_prompt.txt`;
        const promptFilePath = path.join(outputDir, promptFileName);
        fs.writeFileSync(promptFilePath, prompt, "utf-8");
        console.log(`[年度运势总结语] Prompt 已保存到: ${promptFilePath}`);
      } catch (error) {
        console.error("[年度运势总结语] 保存 Prompt 失败:", error);
      }
    });
