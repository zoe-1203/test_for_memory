...
import { getAnnualFortuneAreaOverviewPrompt, LLM_TEMPERATURES } from "@/lib/prompts";
...
export async function POST(req: Request) {
  ...
  try {
    ...
    const prompt = getAnnualFortuneAreaOverviewPrompt(areaInfoText);
    ...
    const requestOptions: any = {
      model,
      temperature: LLM_TEMPERATURES.annualAreaOverview,
      messages: [
        {
          role: "user",
          content: prompt
    ...
    setImmediate(() => {
      try {
        const promptFileName = `areas_overview_${timestamp}_prompt.txt`;
        const promptFilePath = path.join(outputDir, promptFileName);
        fs.writeFileSync(promptFilePath, prompt, "utf-8");
        console.log(`[六大领域总览] Prompt 已保存到: ${promptFilePath}`);
      } catch (error) {
        console.error("[六大领域总览] 保存 Prompt 失败:", error);
      }
    });
