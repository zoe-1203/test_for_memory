...
年运功能共 **4 个步骤**，其中步骤 1 和 3 为并发调用，步骤 4 为并发调用。

---

## 步骤 1：12 个月运势（并发 12 次）
...
| `genderLabel` | string | ❌ | 性别显示文本：`女` / `男` / `不方便告诉` |
...
| 变量名 | 类型 | 必填 | 说明 |
|--------|------|------|------|
| `monthlyContents` | string[] | ✅ | 12 个月的运势内容数组（按月份顺序） |
| `nickName` | string | ❌ | 用户昵称 |
...
| 变量名 | 类型 | 必填 | 说明 |
|--------|------|------|------|
| `area` | string | ✅ | 领域名称，如 `感情`、`事业·学业`。**注意**：当 `areaId === "career"` 时，会根据 `careerStatus` 动态调整：<br>- 学生（`middle_high_school` / `college_above`）→ `学业`<br>- 职场人/自由职业（`worker` / `freelance`）→ `事业` |
...
```json
{
  ...
  "summaryHighlight": "用来回答这一年在这个领域最值得记住的一句话，给可实操更具体的内容"
}
```
...
## 步骤 4：雷达图评分 + 年度总结语 + 玄学指引好物（并发 3 次）
...
| 变量名 | 类型 | 必填 | 说明 |
|--------|------|------|------|
| `areaInfoText` | string | ✅ | 六大领域的解读文本，格式：<br>`【感情】<br>一句话主命题：...<br>解读内容：...<br>关键提醒：...<br><br>【事业·学业】<br>...` |
...
| 变量名 | 类型 | 必填 | 说明 |
|--------|------|------|------|
| `decemberContent` | string | ✅ | 十二月运势的 content 字段 |
| `areaInfoText` | string | ✅ | 六大领域的解读文本（同雷达图） |
...
| 代码值 | 显示文本 | 在事业领域 Prompt 中的值 |
|--------|----------|-------------------------|
| `middle_high_school` | 初高中生 | `学业` |
| `college_above` | 大学及以上 | `学业` |
| `worker` | 职场人 | `事业` |
| `freelance` | 自由职业 | `事业` |
...
1. **步骤 1**：并发调用 12 次 `getAnnualFortunePrompt`，生成 12 个月运势
2. **步骤 2**：调用 1 次 `getAnnualFortuneSummaryPrompt`，生成年度总览
3. **步骤 3**：并发调用 6 次 `getAnnualFortuneAreaPrompt`，生成六大领域解读
4. **步骤 4**：并发调用 3 次：
   - `getAnnualFortuneAreaRadarPrompt`（雷达图评分 + startingOverview）
   - `getAnnualFortuneClosingPrompt`（年度总结语）
   - `getAnnualFortuneGoodLuckItemsPrompt`（玄学指引好物）

---

## 注意事项

1. **area 变量动态调整**：在步骤 3 中，当 `areaId === "career"` 时，`area` 变量会根据 `careerStatus` 自动调整为 `学业` 或 `事业`
2. **startingOverview**：从步骤 4.1 的雷达图 API 返回结果中获取，不再单独调用 overview API
3. **所有 Prompt 输出均为 JSON 格式**
