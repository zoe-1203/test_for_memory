...
**用户的提问的人物和其他信息里提到的人物可能不是同一个人，如果用户的提问没有明确指代某个人，请你不要套用其他信息里的人物
比如如果用户的提问里用的是“她/他/ta”的代称，请你不要套用到其他信息内的人物。可能不是同一个人。
**
...
## 你的口吻
如果用户今天的问题，和之前问过的「用户的其他信息」里用户关注的内容相关，请你结合做出解读。
比如你可以说：“我看到你昨天在关注.../今天早一点在关注...”
...
export function getAnnualFortunePrompt(args: {
  ...
  // 条件变量：判断是否为单身、暧昧或有对象状态
  const isSingle = loveStatus === 'single' || loveStatusLabel?.includes('单身');
  const isAmbiguous = loveStatus === 'ambiguous' || loveStatusLabel?.includes('暧昧');
  const isInRelationship =
    loveStatus === 'in_relationship' ||
    loveStatusLabel?.includes('有对象') ||
    loveStatusLabel?.includes('恋爱');
  
  // 单身用户的感情状态变化提示
  const singleStatusChangeNote = isSingle ? `
因为用户是在${timeInfo}的时候，抽2026年的牌，所以如果月份到了下半年，用户的感情状态你需要结合牌来看是否会发生变化。并且解读要用将来时。
` : '';

  // 单身用户的解读指导
  const singleGuidance = isSingle ? `
## 关于感情方面怎么解读
...
` : '';
  ...
  // 有对象用户的解读指导
  const inRelationshipGuidance = isInRelationship ? `
## 关于有对象的用户怎么解读
